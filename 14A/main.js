...
const getMemAddress = (instruction) => {
...
const getProgramMemorySum = (instructions) => {
  ...
  for (let i = 0; i < instructions.length; i++) {
    ...
    switch (type) {
      ...
      case "mem":
        // MEMORY
        address = getMemAddress(instruction)
        value = getMemBinaryValue(instruction)
        ...
        console.log("Address: " + address + ", Value: " + parseInt(maskedValue, 2))
        // Set the value in the memory address (override value if address already set)
        let findMemory = programMemory.find( x => x.address === address )
        if (findMemory) {
          findMemory.value = parseInt(maskedValue, 2)
        } else {
          programMemory.push({address: address, value: parseInt(maskedValue, 2)})
        }
        break
...
result.innerHTML = "2020 Question 14A - Memory Value Sum = " + getProgramMemorySum(puzzle_instructions)