...
const initRuleSet = (rules_list) => {
  for (let i = 0; i < rules_list.length; i++) {
    const ruleStr = rules_list[i];

    const [lineNumStr, ruleDataStr] = ruleStr.split(": ")
    console.log(lineNumStr + " and " + ruleDataStr)
    
    if (!ruleDataStr.includes('"')) {
      const ruleData = ruleDataStr.split(" ")
      console.log(ruleData)

      let rule = new Array()
      let ruleCount = 0
      rule[ruleCount] = new Array()
      ruleData.forEach(element => {
        if (element == '|') {
          ruleCount++
          rule[ruleCount] = new Array()
        } else {
          rule[ruleCount].push(parseInt(element))
        }
      });
      console.log(rule)
      ruleSet.push({number: lineNumStr, rule: rule, dataSet: false})
    } else {
      let data = ""
      let validData = new Array()
      for (let j = 0; j < ruleDataStr.length; j++) {
        const element = ruleDataStr[j];
        if (element != '"')
          data += element
      }
      validData.push(data)
      ruleSetValidData.push({number: lineNumStr, data: validData})
    }
  }
  console.log(ruleSet)
  console.log(ruleSetValidData)
}

const populateValidData = () => {
  let done = false
  while(!done) {
    let anyDataSet = false
    ruleSet.forEach(element => {
      if (element.dataSet == false) {
        anyDataSet = true
        let dataStr = ""
        let skipElement = false
        //console.log("For line number: " + element.number)
        element.rule.forEach(ruleSubSet => {
          ruleSubSet.forEach(lineNumber => {
            if (!ruleSetValidData.find(x => x.number == lineNumber))
              skipElement = true
          });
        });
        if (!skipElement) {
          console.log("Can set data for number: " + element.number)
          let dataSet = []
          element.rule.forEach(ruleSubSet => {
            let subDataSet = []
            ruleSubSet.forEach(lineNumber => {
              if (subDataSet.length == 0) {
                subDataSet = ruleSetValidData.find(x => x.number == lineNumber).data
              } else {
                let ruleSetData = ruleSetValidData.find(x => x.number == lineNumber).data
                let newSubDataSet = []
                subDataSet.forEach(subData => {
                  ruleSetData.forEach(ruleData => {
                    newSubDataSet.push(subData + ruleData)
                  })
                })
                subDataSet = [...newSubDataSet]
              }
            })
            dataSet = dataSet.concat(subDataSet)
          })
          element.dataSet = true
          ruleSetValidData.push({number: element.number, data: dataSet})
          console.log(dataSet)
        }
      }  
    });
...
const getTotalMatchingRule0 = (rules_list, input_list) => {
  initRuleSet(rules_list)
  populateValidData()
  let rule0ValidData = ruleSetValidData.find(x => x.number == "0").data
  console.log(rule0ValidData)

  let validDataCount = 0
  input_list.forEach(input => {
    for (let i = 0; i < rule0ValidData.length; i++) {
      const data = rule0ValidData[i]
      if (data == input) {
        validDataCount++
        break
      }
...
result.innerHTML = "2020 Question 19A - Number of Lines Matching Rule 0 = " + getTotalMatchingRule0(puzzle_rule_lines, puzzle_input_lines)