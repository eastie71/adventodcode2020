...
const getMemBinaryAddress = (instruction) => {
  let memStr = instruction.slice(4, instruction.indexOf("]"))
  let binaryAddress = parseInt(memStr).toString(2)
  let zeroPadding = ""
  for (let i = 0; i < (MASK_SIZE - binaryAddress.length); i++) {
    zeroPadding += "0" 
  } 
  return zeroPadding + binaryAddress
}

const getMemValue = (instruction) => {
  let value = instruction.slice(instruction.indexOf("=")+2)
...
const getMemBinaryValue = (instruction) => {
  let value = instruction.slice(instruction.indexOf("=")+2)
  // console.log("Value = " + value)
  // Convert to number first
  value = parseInt(value) 
  ...
  let zeroPadding = ""
  for (let i = 0; i < (MASK_SIZE - binaryValue.length); i++) {
    zeroPadding += "0" 
  } 
  return zeroPadding + binaryValue
}

const calcTotalMemoryValue = (memory_values) => {
  let total = 0
  for (let index = 0; index < memory_values.length; index++) {
    const element = memory_values[index]
    total += element.value
  }
  return total
}
...
const getProgramMemorySum = (instructions) => {
  let programMemory = []
  let currentMask = ""

  for (let i = 0; i < instructions.length; i++) {
    const instruction = instructions[i];
    let type = instruction.slice(0,3)
    let address = 0
    let value = 0
    ...
    switch (type) {
      case "mas":
        // MASK
        currentMask = instruction.slice(7)
        break
      case "mem":
        // MEMORY
        address = getMemBinaryAddress(instruction)
        value = getMemValue(instruction)
        ...
        for (let j = 0; j < currentMask.length; j++) {
          ...
          switch (mask) {
            ...
            case '1':
              ...
              break
            case '0':
              maskedAddress += address[j]
              break
            default:
              alert("Unexpected mask value found!")
              return -1
          }
        }
        ...
        let addressPowerOf = getXCount(maskedAddress)
        let addressCount = Math.pow(2, addressPowerOf)

        let binaryStr = ""
        let zeroPadding = ""
        // Perform a Binary count for all possible memory addresses to set
        // Pad the binary string with 0's to match the count of X's in the masked address
        for (let k = 0; k < addressCount; k++) {
          binaryStr = k.toString(2)
          zeroPadding = ""
          for (let i = 0; i < (addressPowerOf - binaryStr.length); i++) {
            zeroPadding += "0" 
          } 
          binaryStr = zeroPadding + binaryStr
        
          let currentBinaryAddress = ""
          let currBinaryPos = binaryStr.length-1
          for (let l = 0; l < maskedAddress.length; l++) {
            const element = maskedAddress[l];
            if (element == 'X') {
              currentBinaryAddress += binaryStr[currBinaryPos]
              currBinaryPos--
            } else {
              currentBinaryAddress += element
            }
          }
          let currentDecimalAddress = parseInt(currentBinaryAddress, 2)
          console.log("Address: " + currentDecimalAddress + ", Value: " + value)
          // Set the value in the memory address (override value if address already set)
          let findMemory = programMemory.find( x => x.address === currentDecimalAddress )
          if (findMemory) {
            findMemory.value = value
          } else {
            programMemory.push({address: currentDecimalAddress, value: value})
          }
        }
        break
    
      default:
        alert("Unexpected instruction found!")
        return -1
    }
  }
  console.log(programMemory)
  return calcTotalMemoryValue(programMemory)
}

result.innerHTML = "2020 Question 14B - Memory Value Sum = " + getProgramMemorySum(puzzle_instructions)