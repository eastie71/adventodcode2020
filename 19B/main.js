...
let ruleSetValidData = new Array()

const rulesToRegexp = {}
const computeRules = (value, rules) => {
  if (value in rulesToRegexp) {
    return rulesToRegexp[value]
  }
  ...
  if (/^".*"$/.test(value)) {
    ...
    result = `(${computeRules(values[0], rules)}|${computeRules(values[1], rules)})`
  } else {
    const keys = value.split(' ')
    result = keys.map(key => computeRules(rules[key], rules)).join('')
  }
  rulesToRegexp[value] = result
  return result
}

let altRuleSet = {}
const alternativeSolvePartA = (rules_list, input_list) => {
  rules_list.forEach(element => {
    const groups = element.split(": ")
    altRuleSet[groups[0]] = groups[1]
  });
  //console.log(altRuleSet)
  computeRules(altRuleSet[0], altRuleSet)
  console.log(rulesToRegexp)

  const mainRule = new RegExp('^' + rulesToRegexp[altRuleSet[0]] + '$')
  console.log(mainRule.source)

  let count = 0
  input_list.forEach(input => {
    //console.log(input)
    if (mainRule.test(input))
      count++
  })
  return count
}

const calcRegexRule42 = (count) => {
  let regexStr = '(' + rulesToRegexp[altRuleSet[42]]
  for (i = 0; i <= count; i++) {
    regexStr += rulesToRegexp[altRuleSet[42]]
  }
  regexStr += ')'
}

const calcRegexRule31 = (count) => {
  let regexStr = '('
  for (i = 0; i <= count; i++) {
    regexStr += rulesToRegexp[altRuleSet[31]]
  }
  regexStr += ')'
}

const getTotalMatchingRule0 = (rules_list, input_list) => {
  rules_list.forEach(element => {
    const groups = element.split(": ")
    altRuleSet[groups[0]] = groups[1]
  });
  ...
  computeRules(altRuleSet[42], altRuleSet)
  computeRules(altRuleSet[31], altRuleSet)
  console.log(rulesToRegexp)

  // I tried ombining manually here - but could not get it to work....
  // let regex42 = rulesToRegexp[altRuleSet[42]]+rulesToRegexp[altRuleSet[42]]
  // let regex31 = rulesToRegexp[altRuleSet[31]]
  // let count = 0
  // for (let i = 0; i < 30; i++) {
  //   let regexStr = calcRegexRule42(i) + calcRegexRule31(i)
  //   const mainRule = new RegExp('^' + regexStr + '$')
  //   input_list.forEach(input => {
  //     //console.log(input)
  //     if (mainRule.test(input))
  //       count++
  //   })
    // regex42 += rulesToRegexp[altRuleSet[42]]
    // regex31 += rulesToRegexp[altRuleSet[31]]
  //}
  // const mainRule = new RegExp('^' + rulesToRegexp[altRuleSet[0]] + '$')
  // console.log(mainRule.source)

  //const mainRule = new RegExp('^('+rulesToRegexp[altRuleSet[42]]+')+('+rulesToRegexp[altRuleSet[31]]+')+$')

  // Rule 0 consists of 8 and 11; Rule 8 consists of 42 or 42 and 8 (hence is Rule 42 N-times)
  // Rule 11 consists of 41 and 31 or 41 and 11 and 31 - hence is 41 and 31 N-times
  // Combining rule 8 and 11 you get 42 N-times and 42 and 31 N-times; which is 42 N-times and 31 N-times (where 42 appears is at least 1 more time than 31)
  // Got this solution from Youtube: https://www.youtube.com/watch?v=UcRsWkIyix0
  const mainRule = new RegExp('^(?<group42>('+rulesToRegexp[altRuleSet[42]]+')+)(?<group31>('+rulesToRegexp[altRuleSet[31]]+')+)$')
  let count = 0
  input_list.forEach(input => {
    //console.log(input)
    const matches = mainRule.exec(input)
    if (matches) {
      const {groups} = matches
      console.log(groups)
      matches42Count = groups.group42.match(new RegExp(rulesToRegexp[altRuleSet[42]], 'g')).length
      matches31Count = groups.group31.match(new RegExp(rulesToRegexp[altRuleSet[31]], 'g')).length

      console.log(matches31Count, matches42Count)
      if (matches42Count > matches31Count)
        count++
    }
...
//result.innerHTML = "2020 Question 19A - Number of Lines Matching Rule 0 = " + alternativeSolvePartA(test_rule_lines1, test_input_lines1)
result.innerHTML = "2020 Question 19B - Number of Lines Matching Rule 0 (after adjust rule 8 and 11) = " + getTotalMatchingRule0(puzzle_rule_lines1, puzzle_input_lines)