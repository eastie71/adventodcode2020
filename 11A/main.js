...
const getAdjacentOccupiedCount = (row_pos, col_pos) => {
  let count = 0
  let fromRow = row_pos == 0 ? 0 : row_pos-1
  let toRow = row_pos >= currentSeatMap.length-1 ? row_pos : row_pos+1
  let fromCol = col_pos == 0 ? 0 : col_pos-1
  let toCol = col_pos >= currentSeatMap[0].length-1 ? col_pos : col_pos+1

  //console.log("For seat: (" + row_pos + "," + col_pos + "), From Row: " + fromRow + ", To Row: " + toRow + ", From Col: " + fromCol + ", To Col: " + toCol)
    ...
    for (let rowNum = 0; rowNum < currentSeatMap.length; rowNum++) {
      ...
      for (let colNum = 0; colNum < rowWidth; colNum++) {
        ...
        switch (seat) {
          ...
          case '#':
            if (getAdjacentOccupiedCount(rowNum, colNum) >= 4) {
              newSeatMap[rowNum] += 'L'
            } else {
              newSeatMap[rowNum] += seat
            }
...
result.innerHTML = "2020 Question 11A - Total Occupied Seats = " + getNumberOfOccupiedSeats(puzzle_seat_map)