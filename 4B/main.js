...
const checkValidPassportDetails  = (passport) => {
  let valid = true
  for (let index = 0; index < data_type_required.length; index++) {
    const dataType = data_type_required[index];
    const value = passport[dataType]
    let nValue
    switch (dataType) {
      case "byr":
        nValue = parseInt(value)
        if (nValue == NaN || nValue < 1920 || nValue > 2002)
          valid = false
        break
      case "iyr":
        nValue = parseInt(value)
        if (nValue == NaN || nValue < 2010 || nValue > 2020)
          valid = false
        break
      case "eyr":
        nValue = parseInt(value)
        if (nValue == NaN || nValue < 2020 || nValue > 2030)
          valid = false
        break
      case "hgt":
        nValue = parseInt(value)
        ...
        if (nValue != NaN) {
          if (value.includes("cm")) {
            if (nValue >= 150 && nValue <= 193)
              valid = true
          } else if (value.includes("in")) {
            if (nValue >= 59 && nValue <= 76)
              valid = true
          }
        }
        break
      ...
      case "ecl":
        valid = false
        for (let index2 = 0; index2 < valid_ecl.length; index2++) {
          if (value == valid_ecl[index2]) {
            valid = true
            break
          }
        }
        break
      case "pid":
        if (value.length != 9 || parseInt(value) == NaN)
          valid = false
        break
      default:
        break
...
const getValidPasswportCount = (passports) => {
  let validPassportCount = 0
  for (let index = 0; index < passports.length; index++) {
    let valid = true
    const passport = passports[index];
    for (let index2 = 0; index2 < data_type_required.length; index2++) {
      const dataType = data_type_required[index2];
      if (!(dataType in passport)) {
        valid = false
        break
      }
    }
    if (valid) {
      if (checkValidPassportDetails(passport))
        validPassportCount++
    }
  }
...
result.innerHTML = "2020 Question 4B - Number of Valid Passports = "  + getValidPasswportCount(puzzle_passports)